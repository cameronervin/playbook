...
- The `/login` route fallback uses the minimal centered Playbook mark and `Loading ...` on a plain `bg-page` background before the login page renders. Once the login page is mounted, provider-fetch loading stays inside the auth card with provider skeleton tiles.
