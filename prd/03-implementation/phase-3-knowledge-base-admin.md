...
|--------|------|-------------|------------|----------|
| ☐ | **Build admin document upload UI/API** — upload supported department docs and create KB records | US-12, US-24 | Admin POST document succeeds -> document record status is uploaded -> audit event records upload -> athlete cannot upload shared KB doc | prd/02-technical-docs/api-specification.md |
| ☐ | **Connect ingestion status lifecycle** — show uploaded/processing/ready/failed states from backend and KB service | US-13 | Upload starts processing -> status transitions to processing -> ready doc is searchable -> failed doc shows reason | prd/02-technical-docs/_kb-service-architecture.md |
