...
Search and rerank logs are structured for operations and eval debugging: they
include request context, model aliases, candidate/result counts, ranking
strategy counts, and rounded score summaries. They must not include raw query
text, chunk text, source URIs, secrets, or full model request/response payloads.
