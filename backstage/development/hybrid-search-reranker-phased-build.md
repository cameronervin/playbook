...
- Implemented: focused KB-service/backend hardening tests now cover acronym and
  paraphrase search routing, duplicate handling, reranker fail-open behavior,
  private conversation-file isolation, success-only document filtering, safe
  structured logging, and backend citation persistence in KB-service order.
- Implemented: the local KB smoke script can directly validate LiteLLM
  `/rerank` and can assert the expected KB-service fail-open result metadata
  when the reranker is intentionally unavailable.
- Deferred: broader golden retrieval/generation eval specs and datasets are
  intentionally postponed until all agentic surfaces are built; the deferral is
  tracked in `backstage/development/tech-debt-tracker.md`.

Scope:

- Add deterministic hardening cases for:
...
- Track deferred shortcuts in `backstage/development/tech-debt-tracker.md`.
...
- Do not build the broader eval suite until the later cross-agent eval pass.
...
- Decide whether to expose ranking diagnostics to admin analytics or keep them
  log/eval-only.
- Build Playbook golden retrieval/generation eval suites after the remaining
  agentic surfaces exist, so the eval pass covers the product holistically.
