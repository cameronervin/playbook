...
## Loader Policy
- Route segments that can suspend must define `loading.tsx` fallbacks that preserve the target screen geometry: chat rail/thread/composer or admin rail/header/cards. The `/login` pre-page route fallback is the exception: use the minimal centered Playbook mark with `Loading ...` on a plain warm-charcoal background, while the mounted login page keeps the auth-card provider skeleton for provider fetch loading. Do not use plain `Loading [page]...` copy as a page fallback.
