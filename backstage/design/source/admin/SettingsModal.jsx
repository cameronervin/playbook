...
function SmSegmented({ value, options, onChange }) {
  return (
...
function ProfilePane({ s, patch }) {
  return (
    <div>
      <Group title="Details">
        <SmField label="Full name"><TextInput value={s.name} onChange={(v) => patch('name', v)} /></SmField>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <SmField label="Job title"><TextInput value={s.title} onChange={(v) => patch('title', v)} /></SmField>
          <SmField label="Department"><TextInput value={s.dept} onChange={(v) => patch('dept', v)} /></SmField>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <SmField label="Email"><TextInput value={s.email} readOnly icon="mail" /></SmField>
          <SmField label="Role"><TextInput value={s.role} readOnly /></SmField>
        </div>
      </Group>
    </div>
  );
