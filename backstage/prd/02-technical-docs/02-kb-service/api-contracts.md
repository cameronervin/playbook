...
Search strategy and reranker controls remain internal service configuration.
Logs may summarize model aliases, candidate counts, ranking strategy counts, and
safe score ranges, but service responses must not expose provider-specific
LiteLLM rerank payloads.
