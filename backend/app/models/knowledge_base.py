...
class KBCollection(Base):
    """Organization-owned knowledge-base collection definition."""

    __tablename__ = "kb_collections"
    __table_args__ = (
        UniqueConstraint("organization_id", "slug", name="uq_kb_collections_org_slug"),
        Index("ix_kb_collections_organization_id", "organization_id"),
        Index("ix_kb_collections_is_active", "is_active"),
    )
...
class KBMetadataTag(Base):
    """Organization-owned global metadata tag preset."""

    __tablename__ = "kb_metadata_tags"
    __table_args__ = (
        UniqueConstraint("organization_id", "slug", name="uq_kb_metadata_tags_org_slug"),
        Index("ix_kb_metadata_tags_organization_id", "organization_id"),
        Index("ix_kb_metadata_tags_is_active", "is_active"),
    )
...
class KBDocumentTag(Base):
    """Assignment from a KB document to a managed metadata tag."""

    __tablename__ = "kb_document_tags"
    __table_args__ = (
        PrimaryKeyConstraint("document_id", "tag_id"),
        Index("ix_kb_document_tags_tag_id", "tag_id"),
    )
    ...
    tag_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("kb_metadata_tags.id", ondelete="CASCADE"),
        nullable=False,
    )
    created_at: Mapped[datetime] = created_at_column()
    tag: Mapped[KBMetadataTag] = relationship(lazy="selectin")
...
class KBDocument(Base):
    ...
    created_at: Mapped[datetime] = created_at_column()
    updated_at: Mapped[datetime] = updated_at_column()
    collection: Mapped[KBCollection | None] = relationship(lazy="selectin")
    tag_links: Mapped[list[KBDocumentTag]] = relationship(
